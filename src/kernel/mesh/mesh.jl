...
function mod_mesh_read_gmsh!(mesh::St_mesh, inputs::Dict)
    ...
    ngl = nglx #SM tmp while working on changes
    
    tot_edges_internal_nodes = mesh.nedges*(ngl-2)
    tot_faces_internal_nodes = mesh.nfaces*(nglx-2)*(ngly-2)
    if mesh.nsd == 2
        tot_vol_internal_nodes   = mesh.nfaces*(nglx-2)*(ngly-2)
    elseif mesh.nsd == 3
        tot_vol_internal_nodes   = mesh.nelem*(nglx-2)*(ngly-2)*(nglz-2)
    end
    
    el_edges_internal_nodes = mesh.NEDGES_EL*(ngl-2)
    el_faces_internal_nodes = mesh.NFACES_EL*(nglx-2)*(ngly-2)
    #el_vol_internal_nodes   = (ngl-2)^(mesh.nsd)
    if mesh.nsd == 2
        el_vol_internal_nodes   = (nglx-2)*(ngly-2)
    elseif mesh.nsd == 3
        el_vol_internal_nodes   = (nglx-2)*(ngly-2)*(nglz-2)
    end
    ...
    if (mesh.nsd == 1)
        nothing #1D grids are built in mod_mesh_build_mesh! and not read.
    elseif (mesh.nsd == 2)
        
        #mesh.connijk::Array{Int64,3} = zeros(Int64, mesh.nelem, mesh.ngl, mesh.ngl)
        mesh.connijk::Array{Int64,3} = zeros(Int64, mesh.nelem, mesh.nglx, mesh.ngly)
        ...
        for iel = 1:mesh.nelem
            ...
            mesh.connijk[iel, 1,    ngly] = mesh.cell_node_ids[iel][1]
            mesh.connijk[iel, nglx, ngly] = mesh.cell_node_ids[iel][3]
            mesh.connijk[iel, nglx,    1] = mesh.cell_node_ids[iel][4]
    ...
    elseif (mesh.nsd == 3)
        ...
        for iel = 1:mesh.nelem
            ...
            mesh.connijk[iel, 1,ngly,1] = mesh.cell_node_ids[iel][4] #1
            mesh.connijk[iel, 1,1,nglz] = mesh.cell_node_ids[iel][1] #10
            mesh.connijk[iel, nglx,1,1] = mesh.cell_node_ids[iel][6] #11
            mesh.connijk[iel, nglx,ngly,1] = mesh.cell_node_ids[iel][8] #5
            mesh.connijk[iel, 1,ngly,nglz] =  mesh.cell_node_ids[iel][3] #4
            mesh.connijk[iel, nglx,1,nglz] = mesh.cell_node_ids[iel][5] #12
            mesh.connijk[iel, nglx,ngly,nglz] = mesh.cell_node_ids[iel][7] #8
        end
        #
        # Fill in elements dictionary needed by NodeOrdering.jl
        #
...
function mod_mesh_build_mesh!(mesh::St_mesh, interpolation_nodes)
    ...
    mesh.connijk::Array{Int64} = zeros(mesh.nelem, mesh.npoin_el, 1)
    
    for iel = 1:mesh.nelem
        mesh.conn[iel, 1] = iel
        mesh.conn[iel, 2] = iel + 1
        
        mesh.connijk[iel, 1, 1] = iel
        mesh.connijk[iel, 2, 1] = iel + 1
    end
