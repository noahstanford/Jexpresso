    ...
    for ieq = 1:params.neqs
        for i = 1:params.mesh.ngl
            ...
            if (ip < params.mesh.npoin)
                ...
                params.RHS[ip,ieq] = (u[ip+1] - u[ip])/(params.mesh.Δx[ip])
            end
        end
    end
...
function _expansion_inviscid!(params, u, iel, ::CL, QT::Inexact, SD::NSD_1D, AD::FD)
    ...
    for i = 1:params.mesh.ngl
        ...
        if (ip < params.mesh.npoin)
            #params.RHS[ip,ieq] = params.visc_coeff[ip]*(u[ip+1] - 2*u[ip] + u[ip-1])/(params.mesh.Δx[ip])^2 + 0.1*(u[ip+1] - u[ip])/(params.mesh.Δx[ip])
            # params.RHS[ip,ieq] = 0.5*(u[ip+1+idx] - u[ip+idx])/(params.mesh.Δx[ip])
            params.RHS[ip,1] = (u[ip+1] - u[ip])/(params.mesh.Δx[ip])#(u[ip+1+params.mesh.npoin] - u[ip+params.mesh.npoin])/(params.mesh.Δx[ip])
